...
- 렉시컬 환경은 스코프 체인을 형성하게 되는데, 스코프 체인은 함수가 선언될 때의 모든 변수와 함수를 포함하는 렉시컬 스코프를 형성

```javascript
function outer() {
  const outerVal = "난 외부 변수야";
...
//outer는 inner를 return 했습니다.innerExecute에는 outer에서 반환한 inner함수가 할당되게 됩니다.
//innerExecute를 호출할 때마다, outer에서 정의한 outerVal을 출력하게 됩니다.
//이때 inner 함수는 outer 함수의 스코프 체인을 유지하게 되는데, 이러한 특성을 이용하여 클로저를 구현한 것입니다.
```
...
- 변수와 함수 선언이 스코프의 최상단으로 올려져 실행되는 것을 의미
...
- 브라우저는 HTML문서를 파싱해서 DOM 트리를 생성, 이 때, HTML 태그를 노드로 변환하고, 노드간의 계층 관계를 형성
- 브라우저는 CSS 파일을 파싱해서 CSSOM 트리를 생성, 이 때, CSS 속성을 노드로 변환하고, 노드간의 계층 관계를 형성
- DOM트리와 CSSOM을 결합하여 렌더링 트리를 생성합니다. 이 때, 실제 화면에 표시될 요소만을 선택하여 렌더링 트리를 형성
- 브라우저는 렌더링 트리를 이용하여 각 요소의 크기와 위치를 계산하는 과정인 레이아웃을 거쳐 화면에 요소를 그리는 <br>
  페인팅 과정을 거치게 된다.
